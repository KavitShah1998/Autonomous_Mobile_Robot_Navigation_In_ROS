...
public:
    ...
    /**
	 * \brief      displays the local grid
     *  
	 */
    void displayLocalGrid();
...
private:
    ...
    std::string _windowName{"Local Occupancy Grid"};

    cv::Mat _occupancyGridImage;

    ros::Subscriber _occupancyGridSubscriber;

    std::vector<std::vector<double>> _bestPathLocal;
    ...
    std::unique_ptr<AStar> _LocalPlannerAStar;


    // name of local planner as stated in planner.yaml
    std::string _LocalPlannerName{"unknown"};
    ...
    /**
	 * \brief      loads the rosparams for local planner
	 */
    void _loadParams();  
	 ...
	 */
    template<typename T>
    std::vector<T> _getSTDVectorPoint(Eigen::Vector3d& eigenPoint);
    ...
    /**
	 * \brief      converts the local path from pixel coords to meters
     * 
