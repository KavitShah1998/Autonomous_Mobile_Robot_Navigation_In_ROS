...
private:
  ...
  std::vector<int> getBestParent_(std::vector<int> neighbourhood); // it returns vector of <i,j,index> of best parent for a given node X_new whose neighbourhood (indices of nearby nodes) is given to it as input args  //SOUMYA
